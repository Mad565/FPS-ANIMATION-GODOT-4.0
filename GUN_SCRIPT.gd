...
func ADS(delta):
	if Input.is_action_pressed("ADS"):
		$Node3D/pp_19_vit/RootNode/LENS/AnimationPlayer.play("ads")
		transform.origin = transform.origin.lerp(ads_pos , ADSLERP * delta)
		camera.fov = lerpf(camera.fov, FOVVIEW["ADS"], ADSLERP * delta)
	else:
		$Node3D/pp_19_vit/RootNode/LENS/AnimationPlayer.play_backwards("RESET")
		transform.origin = transform.origin.lerp(default_pos, ADSLERP * delta)
		camera.fov = lerpf(camera.fov, FOVVIEW["default"], ADSLERP * delta)
